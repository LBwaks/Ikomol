...
class BlogDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        form = CommentForm()
        comments = Comment.objects.filter(blog=blog_to_comment).order_by(
            "-created_date"
        )
...
def search(request):
    ...
    if request.method == "GET":
        q = request.GET.get("q")
        results = (
            Blog.published.annotate(search=SearchVector("title", "description", "tag"))
            .filter(search=q)
    ...
    return render(
        request, "blogs/search.html", {"form": form, "results": results, "q": q}
    )
